...
## 🔧 Additional Modifications for Increased Difficulty

**Note**: The following additional issues were introduced to make the assessment more challenging:

### Dockerfile
- **Removed `WORKDIR /app`** from both builder and runtime stages
  - This will cause the binary and VERSION file to be copied to the wrong location
  - Candidate must recognize the need for proper working directory setup

### Makefile
- **Removed `-o` flag from `go build` command** (line 9)
  - Changed from: `go build -o go-app-demo .`
  - To: `go build go-app-demo .`
  - This creates incorrect syntax and will cause build failures
  - Candidate must fix the build command to properly specify the output binary name

- **Changed `go test` to `go tests`** (line 13)
  - Changed from: `go test -v ./...`
  - To: `go tests -v ./...`
  - The command `go tests` does not exist (correct command is `go test`)
  - Will fail with "unknown command" error
  - Tests ability to debug basic Go tooling errors

- **Removed `-t` flag from `docker build` command** (line 17)
  - Changed from: `docker build -t $(IMAGE_NAME):latest .`
  - To: `docker build $(IMAGE_NAME):latest .`
  - Without `-t`, the positional argument is interpreted as the build context path
  - This will cause "invalid argument" error as it tries to use the image name as the path
  - Candidate must understand Docker build syntax and the purpose of the `-t` (tag) flag
